...
router.post("/login", async (req, res) => {
  try {
    ...
    if (userInfo) {
      ...
      res
        .status(400)
        .json({ error: "Didn't provide a valid username and/or password" });
...
router.post("/signup", async (req, res) => {
  try {
    ...
    if (!body.firstname || !body.lastname || !body.email || !body.password)
      throw "Please provide all input for createUser!";
...
//        appointments: appointments,
      ...
      res
        .status(200)
        .render("function/UserProfile", { user: userinfo, edit: false });
    } catch (e) {
      res
        .status(500)
  ...
  if (req.session && req.session.user) {
    try {
      ...
      if (Object.keys(body).length != 1) {
        ...
        res
          .status(200)
          .render("function/UserProfile", { user: userinfo, edit: true });
      }
    } catch (e) {
      res
        .status(500)
...
router.get("/favorite/:id", async (req, res) => {
  if (req.session && req.session.user) {
    ...
    try {
      const addToProfile = await userData.addfavorite(
        req.session.user.userId,
        req.params.id
      );
      // console.log(`addToProfile: ${addToProfile}`);
      res.redirect("/parks/id/" + req.params.id);
    } catch (e) {
...
router.get("/unfavorite/:id", async (req, res) => {
  if (req.session && req.session.user) {
    ...
    try {
      const removeFromProfile = await userData.removefavorite(
        req.session.user.userId,
        req.params.id
      );
      // console.log(`removeFromProfile: ${removeFromProfile}`);
      res.redirect("/parks/id/" + req.params.id);
    } catch (e) {
      res.status(500).json({ error: e });
    }
